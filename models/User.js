...
var userSchema = mongoose.Schema({
  ...
  password:{
    type:String,
    required:[true,'비밀번호는 필수 입력 사항입니다!'],
    select:false
  },
  name:{
    type:String,
    required:[true,'닉네임은 필수 입력 사항입니다!'],
    match:[/^.{2,12}$/,'2-12자 사이로 입력하세요!'],
    trim:true
  },
  email:{
    type:String,
...
var passwordRegexErrorMessage = '8자 이상의 문자/숫자 조합이 필요합니다!';
userSchema.path('password').validate(function(v) {
  ...
  if(user.isNew){
    if(!user.passwordConfirmation){
      user.invalidate('passwordConfirmation', '비밀번호 재입력이 필요합니다!');
    }
